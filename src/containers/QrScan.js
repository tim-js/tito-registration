...
class QrScan extends Component {
  ...
  _handleBarCodeRead = async qrData => {
    ...
    const [ticketData, checkIns] = await Promise.all([
      ...
      TitoCheckInApi.get(`checkin_lists/${this.props.accountSettings.checkinListSlug}/checkins`)
      ]
    );
  ...
  checkin(modal) {
    ...
    TitoCheckInApi.post(`checkin_lists/${this.props.accountSettings.checkinListSlug}/checkins`, {checkin: { ticket_id: ticketId}}).then(() => {
      modal.hideModal();
      this.props.navigation.navigate('CheckInList', { checkinListSlug: this.state.checkinListSlug });
    }).catch(error => {
      console.log(error);
        ...
        <Modal
          ...
          <View style={{ flex: 1,
            ...
            { this.state.isLoading ? (
              ...
              <>
                {QrScan._renderTicket(this.state.ticket, this.state.checkinAvailable)}
  ...
  static _renderTicket(ticket, checkinAvailable) {
    ...
    return (
      <>
        ...
        <Text style={{ fontSize: 30 }}>{first_name} {last_name}</Text>
      </>
    );
  }
}
